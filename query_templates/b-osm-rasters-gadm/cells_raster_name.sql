DROP TABLE IF EXISTS {{public_schema}}.cells_{{raster_name}} CASCADE;


CREATE TABLE {{public_schema}}.cells_{{raster_name}} AS (
                          SELECT
                          ...
                          FROM
                              {{public_schema}}.raster_polygons_{{raster_name}} AS raster
                              LEFT JOIN {{public_schema}}.gadm36 AS gadm ON ST_Contains(gadm.geom, ST_Centroid(raster.geom))
                              LEFT JOIN {{public_schema}}.country_codes AS cc ON gadm.name_0 = cc.gadm_country
                          );


--adding another column to denote whether centroid falls on continental land or not
ALTER TABLE {{public_schema}}.cells_{{raster_name}}
    ADD COLUMN centroid_on_land BOOLEAN;


UPDATE {{public_schema}}.cells_{{raster_name}} SET
...
CREATE INDEX cells_{{raster_name}}_gidx ON {{public_schema}}.cells_{{raster_name}} USING gist(cell_geom);
CREATE INDEX cells_{{raster_name}}_centroid_gidx ON {{public_schema}}.cells_{{raster_name}} USING gist(cell_centroid);


VACUUM ANALYZE {{public_schema}}.cells_{{raster_name}};