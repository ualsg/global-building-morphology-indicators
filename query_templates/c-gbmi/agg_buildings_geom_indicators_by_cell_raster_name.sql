...
CREATE TABLE {{gbmi_schema}}.agg_buildings_geom_indicators_by_cell_{{raster_name}} AS (
                                                                  WITH agg1 AS (
                                                                               SELECT
                                                                                   ...
                                                                                   cell_admin_div1,
                                                                                   cell_admin_div2,
                                                                                   count(osm_id) / cell_area AS count_buildings,
                                                                                   min(clipped_bldg_area) / cell_area AS min_footprint_area,
                                                                                   (percentile_cont(0.5) WITHIN GROUP (ORDER BY clipped_bldg_area)) / cell_area AS median_footprint_area,
                                                                                   avg(clipped_bldg_area) / cell_area AS mean_footprint_area,
                                                                                   max(clipped_bldg_area) / cell_area AS max_footprint_area,
                                                                                   sum(clipped_bldg_area) / cell_area AS sum_footprint_area,
                                                                                   stddev_pop(clipped_bldg_area) / cell_area AS sd_footprint_area,
                                                                                   min(clipped_bldg_perimeter) / cell_area AS min_perimeter,
                                                                                   (percentile_cont(0.5) WITHIN GROUP (ORDER BY clipped_bldg_perimeter)) / cell_area AS median_perimeter,
                                                                                   avg(clipped_bldg_perimeter) / cell_area AS mean_perimeter,
                                                                                   max(clipped_bldg_perimeter) / cell_area AS max_perimeter,
                                                                                   sum(clipped_bldg_perimeter) / cell_area AS sum_perimeter,
                                                                                   stddev_pop(clipped_bldg_perimeter) / cell_area AS sd_perimeter,
                                                                                   min(height) / cell_area AS min_height,
                                                                                   (percentile_cont(0.5) WITHIN GROUP (ORDER BY height)) / cell_area AS median_height,
                                                                                   avg(height) / cell_area AS mean_height,
                                                                                   max(height) / cell_area AS max_height,
                                                                                   sum(height) / cell_area AS sum_height,
                                                                                   stddev_pop(height) / cell_area AS sd_height,
                                                                                   min(est_volume) / cell_area AS min_volume,
                                                                                   (percentile_cont(0.5) WITHIN GROUP (ORDER BY est_volume)) / cell_area AS median_volume,
                                                                                   avg(est_volume) / cell_area AS mean_volume,
                                                                                   max(est_volume) / cell_area AS max_volume,
                                                                                   sum(est_volume) / cell_area AS sum_volume,
                                                                                   stddev_pop(est_volume) / cell_area AS sd_volume,
                                                                                   min(est_wall_area) / cell_area AS min_wall_area,
                                                                                   (percentile_cont(0.5) WITHIN GROUP (ORDER BY est_wall_area)) / cell_area AS median_wall_area,
                                                                                   avg(est_wall_area) / cell_area AS mean_wall_area,
                                                                                   max(est_wall_area) / cell_area AS max_wall_area,
                                                                                   sum(est_wall_area) / cell_area AS sum_wall_area,
                                                                                   stddev_pop(est_wall_area) / cell_area AS sd_wall_area,
                                                                                   min(est_envelope_area) / cell_area AS min_envelope_area,
                                                                                   (percentile_cont(0.5) WITHIN GROUP (ORDER BY est_envelope_area)) / cell_area AS median_envelope_area,
                                                                                   avg(est_envelope_area) / cell_area AS mean_envelope_area,
                                                                                   max(est_envelope_area) / cell_area AS max_envelope_area,
                                                                                   sum(est_envelope_area) / cell_area AS sum_envelope_area,
                                                                                   stddev_pop(est_envelope_area) / cell_area AS sd_envelope_area,
                                                                                   min(est_floor_area) / cell_area AS min_floor_area,
                                                                                   (percentile_cont(0.5) WITHIN GROUP (ORDER BY est_floor_area)) / cell_area AS median_floor_area,
                                                                                   avg(est_floor_area) / cell_area AS mean_floor_area,
                                                                                   max(est_floor_area) / cell_area AS max_floor_area,
                                                                                   sum(est_floor_area) / cell_area AS sum_floor_area,
                                                                                   stddev_pop(est_floor_area) / cell_area AS sd_floor_area,
                                                                                   min(count_vertices) / cell_area AS min_count_vertices,
                                                                                   (percentile_cont(0.5) WITHIN GROUP (ORDER BY count_vertices)) / cell_area AS median_count_vertices,
                                                                                   avg(count_vertices) / cell_area AS mean_count_vertices,
                                                                                   max(count_vertices) / cell_area AS max_count_vertices,
                                                                                   sum(count_vertices) / cell_area AS sum_count_vertices,
                                                                                   stddev_pop(count_vertices) / cell_area AS sd_count_vertices,
                                                                                   ...
                                                                                   min(est_floor_area) FILTER (WHERE is_residential IS TRUE) / cell_area AS min_residential_floor_area,
                                                                                   (percentile_cont(0.5) WITHIN GROUP (ORDER BY est_floor_area) FILTER (WHERE is_residential IS TRUE)) / cell_area AS median_residential_floor_area,
                                                                                   avg(est_floor_area) FILTER (WHERE is_residential IS TRUE) / cell_area AS mean_residential_floor_area,
                                                                                   max(est_floor_area) FILTER (WHERE is_residential IS TRUE) / cell_area AS max_residential_floor_area,
                                                                                   sum(est_floor_area) FILTER (WHERE is_residential IS TRUE) / cell_area AS sum_residential_floor_area,
                                                                                   stddev_pop(est_floor_area) FILTER (WHERE is_residential IS TRUE) / cell_area AS sd_residential_floor_area,
                                                                  ...
                                                                  SELECT
                                                                      ...
                                                                      sd_floor_area,
                                                                      CASE
                                                                          WHEN mean_floor_area > 0
                                                                              THEN (sd_floor_area) ^ 2 / mean_floor_area
                                                                      END AS d_floor_area,
                                                                      CASE
                                                                      ...
                                                                      sd_complexity,
                                                                      CASE
                                                                          WHEN mean_complexity > 0
                                                                              THEN (sd_complexity) ^ 2 / mean_complexity
                                                                      END AS d_complexity,
                                                                      CASE
                                                                      ...
                                                                      sd_mbr_length,
                                                                      CASE
                                                                          WHEN mean_mbr_length > 0
                                                                              THEN (sd_mbr_length) ^ 2 / mean_mbr_length
                                                                      END AS d_mbr_length,
                                                                      CASE
                                                                      ...
                                                                      sd_mbr_width,
                                                                      CASE
                                                                          WHEN mean_mbr_width > 0
                                                                              THEN (sd_mbr_width) ^ 2 / mean_mbr_width
                                                                      END AS d_mbr_width,
                                                                      CASE
                                                                      ...
                                                                      sd_mbr_area,
                                                                      CASE
                                                                          WHEN mean_mbr_area > 0
                                                                              THEN (sd_mbr_area) ^ 2 / mean_mbr_area
                                                                      END AS d_mbr_area,
                                                                      CASE
                                                                      ...
                                                                      sd_start_date,
                                                                      CASE
                                                                          WHEN mean_start_date > 0
                                                                              THEN (sd_start_date) ^ 2 / mean_start_date
                                                                      END AS d_start_date,
                                                                      CASE
