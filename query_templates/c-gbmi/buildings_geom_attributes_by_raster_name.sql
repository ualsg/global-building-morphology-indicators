...
CREATE TABLE {{gbmi_schema}}.buildings_geom_attributes_by_{{raster_name}} AS (
                                 WITH clipped AS (
                                                 SELECT
                                                    ...
                                                    (ST_DUMP(ST_INTERSECTION(cells.cell_geom, bga.way))).geom AS clipped_way
                                                 FROM
                                                    {{db_schema}}.cells_{{raster_name}} AS cells
                                                    INNER JOIN {{gbmi_schema}}.buildings_geom_attributes bga ON ST_INTERSECTS(cells.cell_geom, bga.way)
                                                 )
                                 SELECT *,
