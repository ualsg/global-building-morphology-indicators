DROP TABLE IF EXISTS {{gbmi_schema}}.agg_buildings_geom_indicators_by_admin_div1_{{raster_name}};

CREATE TABLE {{gbmi_schema}}.agg_buildings_geom_indicators_by_admin_div1_{{raster_name}} AS (
                                                                        WITH agg_areas AS (
                                                                                          SELECT DISTINCT
                                                                                              country,
                                                                                              admin_level1,
                                                                                              sum(area_sqm) OVER ( PARTITION BY country, admin_level1 ) AS agg_area
                                                                                          FROM
                                                                                              {{db_schema}}.agg_gadm36_country_admin_level_areas agcala
                                                                                          ),
                                                                            agg1 AS (
                                                                                    SELECT
                                                                                        cell_country,
                                                                                        cell_admin_div1,
                                                                                        count(osm_id) / aa.agg_area AS count_buildings,
                                                                                        min(clipped_bldg_area) / aa.agg_area AS min_footprint_area,
                                                                                            (percentile_cont(0.5) WITHIN GROUP ( ORDER BY clipped_bldg_area )) /
                                                                                            aa.agg_area AS median_footprint_area,
                                                                                        avg(clipped_bldg_area) / aa.agg_area AS mean_footprint_area,
                                                                                        max(clipped_bldg_area) / aa.agg_area AS max_footprint_area,
                                                                                        sum(clipped_bldg_area) / aa.agg_area AS sum_footprint_area,
                                                                                        stddev_pop(clipped_bldg_area) / aa.agg_area AS sd_footprint_area,
                                                                                        min(clipped_bldg_perimeter) / aa.agg_area AS min_perimeter,
                                                                                            (percentile_cont(0.5) WITHIN GROUP ( ORDER BY clipped_bldg_perimeter )) /
                                                                                            aa.agg_area AS median_perimeter,
                                                                                        avg(clipped_bldg_perimeter) / aa.agg_area AS mean_perimeter,
                                                                                        max(clipped_bldg_perimeter) / aa.agg_area AS max_perimeter,
                                                                                        sum(clipped_bldg_perimeter) / aa.agg_area AS sum_perimeter,
                                                                                        stddev_pop(clipped_bldg_perimeter) / aa.agg_area AS sd_perimeter,
                                                                                        min(height) / aa.agg_area AS min_height,
                                                                                        (percentile_cont(0.5) WITHIN GROUP ( ORDER BY height )) / aa.agg_area AS median_height,
                                                                                        avg(height) / aa.agg_area AS mean_height,
                                                                                        max(height) / aa.agg_area AS max_height,
                                                                                        sum(height) / aa.agg_area AS sum_height,
                                                                                        stddev_pop(height) / aa.agg_area AS sd_height,
                                                                                        min(est_volume) / aa.agg_area AS min_volume,
                                                                                        (percentile_cont(0.5) WITHIN GROUP ( ORDER BY est_volume )) / aa.agg_area AS median_volume,
                                                                                        avg(est_volume) / aa.agg_area AS mean_volume,
                                                                                        max(est_volume) / aa.agg_area AS max_volume,
                                                                                        sum(est_volume) / aa.agg_area AS sum_volume,
                                                                                        stddev_pop(est_volume) / aa.agg_area AS sd_volume,
                                                                                        min(est_wall_area) / aa.agg_area AS min_wall_area,
                                                                                            (percentile_cont(0.5) WITHIN GROUP ( ORDER BY est_wall_area )) /
                                                                                            aa.agg_area AS median_wall_area,
                                                                                        avg(est_wall_area) / aa.agg_area AS mean_wall_area,
                                                                                        max(est_wall_area) / aa.agg_area AS max_wall_area,
                                                                                        sum(est_wall_area) / aa.agg_area AS sum_wall_area,
                                                                                        stddev_pop(est_wall_area) / aa.agg_area AS sd_wall_area,
                                                                                        min(est_envelope_area) / aa.agg_area AS min_envelope_area,
                                                                                            (percentile_cont(0.5) WITHIN GROUP ( ORDER BY est_envelope_area )) /
                                                                                            aa.agg_area AS median_envelope_area,
                                                                                        avg(est_envelope_area) / aa.agg_area AS mean_envelope_area,
                                                                                        max(est_envelope_area) / aa.agg_area AS max_envelope_area,
                                                                                        sum(est_envelope_area) / aa.agg_area AS sum_envelope_area,
                                                                                        stddev_pop(est_envelope_area) / aa.agg_area AS sd_envelope_area,
                                                                                        min(est_floor_area) / aa.agg_area AS min_floor_area,
                                                                                            (percentile_cont(0.5) WITHIN GROUP ( ORDER BY est_floor_area )) /
                                                                                            aa.agg_area AS median_floor_area,
                                                                                        avg(est_floor_area) / aa.agg_area AS mean_floor_area,
                                                                                        max(est_floor_area) / aa.agg_area AS max_floor_area,
                                                                                        sum(est_floor_area) / aa.agg_area AS sum_floor_area,
                                                                                        stddev_pop(est_floor_area) / aa.agg_area AS sd_floor_area,
                                                                                        min(count_vertices) / aa.agg_area AS min_count_vertices,
                                                                                            (percentile_cont(0.5) WITHIN GROUP ( ORDER BY count_vertices )) /
                                                                                            aa.agg_area AS median_count_vertices,
                                                                                        avg(count_vertices) / aa.agg_area AS mean_count_vertices,
                                                                                        max(count_vertices) / aa.agg_area AS max_count_vertices,
                                                                                        sum(count_vertices) / aa.agg_area AS sum_count_vertices,
                                                                                        stddev_pop(count_vertices) / aa.agg_area AS sd_count_vertices,
                                                                                        ...
                                                                                        min(est_floor_area) FILTER ( WHERE is_residential IS TRUE ) / aa.agg_area AS min_residential_floor_area,
                                                                                            (percentile_cont(0.5)
                                                                                             WITHIN GROUP ( ORDER BY est_floor_area )
                                                                                             FILTER ( WHERE is_residential IS TRUE )) /
                                                                                            aa.agg_area AS median_residential_floor_area,
                                                                                        avg(est_floor_area) FILTER ( WHERE is_residential IS TRUE ) / aa.agg_area AS mean_residential_floor_area,
                                                                                        max(est_floor_area) FILTER ( WHERE is_residential IS TRUE ) / aa.agg_area AS max_residential_floor_area,
                                                                                        sum(est_floor_area) FILTER ( WHERE is_residential IS TRUE ) / aa.agg_area AS sum_residential_floor_area,
                                                                                            stddev_pop(est_floor_area) FILTER ( WHERE is_residential IS TRUE ) /
                                                                                            aa.agg_area AS sd_residential_floor_area,
                                                                                    ...
                                                                                    FROM
                                                                                        ...
                                                                                        LEFT JOIN agg_areas aa
                                                                                        ON bga.cell_country = aa.country AND bga.cell_admin_div1 = aa.admin_level1
                                                                                    GROUP BY bga.cell_country, cell_admin_div1, aa.agg_area
                                                                                    )
                                                                        SELECT
                                                                            cell_country,
                                                                            cell_admin_div1,
