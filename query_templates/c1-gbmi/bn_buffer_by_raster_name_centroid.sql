...
CREATE TABLE {{gbmi_schema}}.bn_{{buffer}}_by_{{raster_name}}_centroid AS (
...
CREATE INDEX bn_{{buffer}}_by_{{raster_name}}_centroid_osm_id ON {{gbmi_schema}}.bn_{{buffer}}_by_{{raster_name}}_centroid(osm_id);

CREATE INDEX bn_{{buffer}}_by_{{raster_name}}_centroid_centroid_spgist ON {{gbmi_schema}}.bn_{{buffer}}_by_{{raster_name}}_centroid USING spgist(way_centroid);

CREATE INDEX bn_{{buffer}}_by_{{raster_name}}_centroid_spgist ON {{gbmi_schema}}.bn_{{buffer}}_by_{{raster_name}}_centroid USING spgist(way);

VACUUM ANALYZE {{gbmi_schema}}.bn_{{buffer}}_by_{{raster_name}}_centroid;
...
CREATE MATERIALIZED VIEW {{gbmi_schema}}.bn_{{buffer}}_by_{{raster_name}}_centroid_duplicates AS
    SELECT
        ({{gbmi_schema}}.bn_{{buffer}}_by_{{raster_name}}_centroid.*)::text,
        count(*)
    FROM
        {{gbmi_schema}}.bn_{{buffer}}_by_{{raster_name}}_centroid
    GROUP BY
        {{gbmi_schema}}.bn_{{buffer}}_by_{{raster_name}}_centroid.*
    HAVING count(*) > 1;
