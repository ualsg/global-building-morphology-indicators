...
CREATE TABLE gbmi.rast_agg_bni_by_cell_{{raster_name}}_attr_abbr AS
    SELECT
        st_union(st_asraster(a.\"c_geom\", b.rast, 'band_pix_type', a.\"attr_abbr\", -9999)) AS rast
    FROM
        (SELECT \"cell_geom\" AS \"c_geom\", \"attr\"::psql_dtype AS \"attr_abbr\" FROM gbmi.agg_bni_by_cell_{{raster_name}}) AS a,
...
CREATE TABLE gbmi.attr_abbr_out AS
    ...
    FROM
        gbmi.rast_agg_bni_by_cell_{{raster_name}}_attr_abbr;


SELECT
    lo_export(loid, 'export_dir/rast_agg_bni_by_cell_{{raster_name}}_attr_abbr.tiff')
FROM
    gbmi.attr_abbr_out;
